        ...
        for action in Directions.CARDINAL:
            ...
            if not(hitsWall):
                ...
                if nextposition in self.corners and nextposition not in visitedcorners:
                    new_visitedcorners = visitedcorners + (nextposition,)
                    cost = 1
                    nextstate = (nextposition, new_visitedcorners)
...
def cornersHeuristic(state, problem):
    ...
    unvisited_corners = [c for c in corners if c not in visited_corners]

    if not unvisited_corners:
        return 0  # All corners visited, so the goal is reached

    distances = [distance.manhattan(position, c) for c in unvisited_corners]
...
def foodHeuristic(state, problem):
    ...
    if not foodList:
        return 0
    h = [(food, distance.euclidean(position, food)) for food in foodList]
    h.sort(key = lambda h: h[1])
    if (len(h) < 2):
        furthest = h[-1]
        return furthest[1]
    furthest = h[-1]
    closest = h[0]
    first_dist = distance.euclidean(position, closest[0])
    second_dist = distance.euclidean(closest[0], furthest[0])
